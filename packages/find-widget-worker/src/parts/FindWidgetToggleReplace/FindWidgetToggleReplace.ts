...
export const toggleReplace = (state: FindWidgetState): FindWidgetState => {
  const { fontSize, inputBorderWidth, inputGap, inputLineHeight, inputPaddingBottom, inputPaddingTop, replaceExpanded, value } = state
  const newExpanded = !replaceExpanded
  const { height } = measureInputHeight(
    value,
    inputLineHeight,
    inputPaddingBottom,
    inputPaddingTop,
