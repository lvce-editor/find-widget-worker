...
export const name = 'find-widget-100k-lines'

export const test: Test = async ({ Editor, expect, FileSystem, FindWidget, Locator, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const lines = Array.from({ length: 100_000 }, (_, index) => (index % 100 === 0 ? `needle ${index}` : `line ${index}`))
  await FileSystem.writeFile(`${tmpDir}/large-file-100k.txt`, lines.join('\n'))
  await setWorkspacePath(Workspace, tmpDir)
  await Main.openUri(`${tmpDir}/large-file-100k.txt`)
  ...
  await expect(matchCount).toHaveText('1 of 1000')

  await FindWidget.focusNext()
  await expect(matchCount).toHaveText('2 of 1000')
